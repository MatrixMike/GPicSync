...
#
# Contributors,
# see: https://github.com/metadirective/GPicSync/graphs/contributors
#
...
# print ">>>>> self.sidecarFile",self.sidecarFile
    ...
    def readExifAll(self):
        ...
        result = os.popen('%s -n "%s"' %
                          (self.exifcmd, self.picPath)).read()
        return result
...
# result=os.popen('exiftool.exe -CreateDate "%s"' % self.picPath).read()
        result = os.popen('%s -DateTimeOriginal "%s"' %
                          (self.exifcmd, self.picPath)).read()
    ...
    def readDateTimeSize(self):
        ...
        """
        answer = os.popen('%s -DateTimeOriginal -ImageSize "%s"' %
                          (self.exifcmd, self.picPath)).read()
...
# timeDate= [answer[34:44],answer[45:53]]
        try:
            size = answer.split("Image Size")[1].split(":")
            [1].strip().split("x")
            width = size[0]
            height = size[1]
        except:
    ...
    def readLongitude(self):
        ...
        result = os.popen('%s -n -GPSLongitude \
        -GPSLongitudeRef   "%s" ' %
                          (self.exifcmd, self.picPath)).read().split("\n")
        # print result
        if len(result) > 1:
...
# os.popen('exiftool.exe -GPSAltitudeRef=0 -GPSAltitude=100 '+ self.picPath)
        if float(lat) >= 0:
            os.popen('%s -m -GPSLatitudeRef="N" %s "%s" ' %
                     (self.exifcmd, option, self.picPath))
        else:
            os.popen('%s -m -GPSLatitudeRef="S" %s "%s" ' %
                     (self.exifcmd, option, self.picPath))
        os.popen('%s  -m -GPSLatitude=%s "%s"' %
                 (self.exifcmd, lat, self.picPath))

    def writeLongitude(self, long):
        """
        write the longitude value given in argument in the EXIF
        positive means Eastern longitudes
        negative means Western latitudes
        ...
        if self.xmpOption:
            if(self.sidecarFile != ""):
                option = option + " -o '"+self.sidecarFile+"'"
        if float(long) >= 0:
            os.popen('%s -m -GPSLongitudeRef="E" %s "%s"' %
                     (self.exifcmd, option, self.picPath))
        else:
            os.popen('%s -m -GPSLongitudeRef="W" %s "%s"' %
                     (self.exifcmd, option, self.picPath))
        os.popen('%s -m -GPSLongitude=%s  "%s" ' %
                 (self.exifcmd, long, self.picPath))

    def writeLatLong(self, lat, long, latRef,
                     longRef, backup, elevation="None"):
        """
        Write both latitudeRef/latitude and
        longitudeRef/longitude in EXIF
        """
        ...
        if backup:
            if elevation == "None":
                ...
                -GPSLongitudeRef=%s -GPSLatitudeRef=%s  %s "%s" '
                         % (self.exifcmd, long, lat, longRef,
                            latRef, option, self.picPath))
            else:
                os.popen('%s -n -m -GPSLongitude=%s \
                -GPSLatitude=%s -GPSLongitudeRef=%s \
                -GPSLatitudeRef=%s  -GPSAltitudeRef=%s \
                -GPSAltitude=%s %s "%s" '
                         % (self.exifcmd, long, lat, longRef,
                            latRef, altRef, elevation, option, self.picPath))
        else:
            if elevation == "None":
                os.popen('%s -m -overwrite_original \
                -n -GPSLongitude=%s \
                -GPSLatitude=%s \
                -GPSLongitudeRef=%s -GPSLatitudeRef=%s  %s "%s" '
                         % (self.exifcmd, long, lat, longRef,
                ...
                os.popen('%s -m -overwrite_original -n \
                -GPSLongitude=%s \
                -GPSLatitude=%s \
                -GPSLongitudeRef=%s -GPSLatitudeRef=%s \
                -GPSAltitudeRef=%s -GPSAltitude=%s %s "%s"'
                         % (self.exifcmd, long, lat, longRef, latRef,
                            altRef, elevation, option, self.picPath))
